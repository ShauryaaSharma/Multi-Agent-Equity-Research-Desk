# Deploy AERD — Vercel + Render (free tier)
...
- [ ] No secrets in Git (`api_core/.env`, `console/.env.local` must not be tracked)
...
| Start Command | `daphne -b 0.0.0.0 -p $PORT aerd_conf.asgi:application` |
| Plan | Free |
...
Copy API URL: `https://aerd-api.onrender.com` (yours will differ).
...
`vercel.json` runs `scripts/edge-db-sync.sh` on each build. On a shared Postgres (Django + Prisma), the first deploy baselines Prisma migrations (P3005); later builds apply new migrations normally.
...
Commit only `.env.example` and `console/.env.local.example`.  
...
For local shared Postgres: `docker compose -f stack.edge.yml up -d` and set `DATABASE_URL` in both env files.
