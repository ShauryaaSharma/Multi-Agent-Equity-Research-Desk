...
from .tone_scoring import analyze_financial_sentiment
from . import finnhub_gateway as fh
from . import newsapi_gateway as na
...
def analyze_sentiment(request):
    ...
    try:
        ...
        if getattr(settings, "FEATURE_GENAI_INSIGHTS", False):
            try:
                from cognition.briefings import build_genai_insights
...
def quant_catalog(request):
    ...
    from alpha_lab.registry import get_full_catalog
...
def quant_signals(request):
    ...
    try:
        ...
        import pandas as pd
        from alpha_lab.triggers import build_signal_payload, sentiment_score_from_probs
...
def quant_backtest_compile(request):
    ...
    try:
        from alpha_lab.playbook_engine import parse_strategy_prompt
        from alpha_lab.playbook_llm import compile_strategy
...
def quant_research_benchmark(request):
    ...
    try:
        from alpha_lab.bench_harness import run_research_benchmark
...
def quant_backtest(request):
    ...
    if str(qp.get("templates", "")).lower() in ("1", "true", "yes"):
        from alpha_lab.playbook_engine import list_templates
        from alpha_lab.event_replay import check_options_chain_available
        from alpha_lab.registry import get_full_catalog
    ...
    if str(qp.get("suggest", "")).lower() in ("1", "true", "yes"):
        from alpha_lab.playbook_llm import groq_suggest_next_words, suggest_phrases
    ...
    if mode in ("equity_intraday", "equity_delivery", "options", "intraday", "delivery"):
        ...
        try:
            from alpha_lab.event_replay import run_event_backtest
...
def evaluation_sentiment_accuracy(request):
    ...
    try:
        from scorecard.gauges import sentiment_accuracy
...
def evaluation_latency(request):
    ...
    try:
        from scorecard.gauges import latency_benchmark
        from newswire.tone_scoring import analyze_financial_sentiment
...
def symbol_deep_dive(request):
    ...
    try:
        from swarm.ticker_probe import SymbolDeepDiveAgent
...
def cross_domain_news(request):
    ...
    try:
        from crosslink.feeds import fetch_domain_news, cross_domain_reasoning
