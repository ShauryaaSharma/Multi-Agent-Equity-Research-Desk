...
# Render build — run from api_core/ (rootDir)
cd "$(dirname "$0")"
...
from jolt_radar.models import ShockEvent
raise SystemExit(0 if ShockEvent.objects.exists() else 1)
" 2>/dev/null || python manage.py replay_jolts --fast --skip-newsapi --threshold 100 --indices nifty,banknifty
