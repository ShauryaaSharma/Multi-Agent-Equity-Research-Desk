...
cd Autonomous-Equity-Research-Desk/console
...
├── components/
│   ├── swarm/            # AI agent cards
│   ├── plots/             # Trading & sentiment charts
│   ├── wire/              # News feed components
│   ├── tape/              # Market ticker & widgets
│   └── ui/                # shadcn components
├── store/
│   ├── tapeStore.ts       # Market data (Zustand)
│   └── swarmStore.ts      # Agent insights (Zustand)
├── hooks/
│   ├── useTapeStream.ts # WebSocket for prices
│   └── useWireFeed.ts     # News data with WS
├── lib/
...
1. **Start Django server** on port 8000:
   ```bash
   cd Autonomous-Equity-Research-Desk/api_core
