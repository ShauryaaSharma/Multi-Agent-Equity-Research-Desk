...
import { apiClient, AgentsRunResult } from '@/lib/httpClient';
import { useSwarmStore, SwarmInsight } from '@/store/swarmStore';

function signalFromText(s: string): SwarmInsight['signal'] {
...
function mapAgentsResultToInsights(result: AgentsRunResult | null): SwarmInsight[] {
  if (!result) return [];
...
export type SwarmInsightsOptions = {
  selectedIndicators?: string[];
  selectedPatterns?: string[];
};

/** Fetches agent insights from /api/agents/run/ and updates agent store */
export function useSwarmInsights(ticker?: string, options?: SwarmInsightsOptions) {
  const setInsights = useSwarmStore((state) => state.setInsights);
