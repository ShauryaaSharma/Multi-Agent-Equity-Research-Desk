...
const src = process.argv[2] || path.join(process.env.HOME, 'Downloads/aerd-premium_1.html');
...
chunk = chunk
  ...
  .replace(/\s+onclick="[^"]*"/g, '');
const out = path.join(__dirname, '../components/aerd/entry-markup.ts');
fs.writeFileSync(
  out,
  `/** Auto-generated from AERD design HTML — do not edit by hand */\nexport const ENTRY_MARKUP = ${JSON.stringify(chunk)};\n`
);
