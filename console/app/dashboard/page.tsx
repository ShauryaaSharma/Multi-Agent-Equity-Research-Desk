...
import { useSwarmInsights } from '@/hooks/useSwarmInsights';
import { useSwarmStore } from '@/store/swarmStore';
import { useTapeStore } from '@/store/tapeStore';
import { apiClient } from '@/lib/httpClient';
import { fmtPct, fmtPrice } from '@/lib/aerd/format';
import { AerdChart } from '@/components/aerd/AerdChart';
import { WireRows } from '@/components/aerd/WireRows';
import { ConductorList } from '@/components/aerd/ConductorList';
import { SwarmCardGrid } from '@/components/aerd/SwarmCardGrid';
...
export default function DashboardPage() {
  ...
  const { news, isLoading: newsLoading } = useWireFeed();
  const insights = useSwarmStore((s) => s.insights);
  const { isLoading: agentsLoading, result: agentsResult } = useSwarmInsights();
      ...
      <div className="row mb14">
        <div className="f1">
          ...
          <div className="card mb14">
            ...
            {newsLoading ? <p className="cb" style={{ color: 'var(--text-3)' }}>Loading news…</p> : <WireRows items={news} limit={5} />}
          </div>

          <div className="pg-sub mb14">🤖 Agent Results</div>
          <SwarmCardGrid insights={insights} />
        </div>
        ...
        <div style={{ width: 310, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="card">
            ...
            <ConductorList steps={agentsResult?.pipeline} isLoading={agentsLoading} />
          </div>
          <div className="card">
            <div className="ch">
