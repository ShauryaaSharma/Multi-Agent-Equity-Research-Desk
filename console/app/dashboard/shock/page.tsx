...
import { shockWebSocketUrl } from '@/lib/socketOrigin';
import type { JoltAlertItem } from '@/components/jolt/JoltAlertFeed';
import type { ShockHistoryRow } from '@/lib/httpClient';
...
export default function ShockPage() {
  ...
  const [alerts, setAlerts] = useState<JoltAlertItem[]>([]);
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      const data = JSON.parse(event.data) as JoltAlertItem & { score?: number; cause?: string };
              ...
              {history.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ textAlign: 'center', color: 'var(--text-3)' }}>
                    No events (try lower threshold). Run: python manage.py replay_jolts --threshold {threshold}
                  </td>
                </tr>
              ) : (
                history.map((e) => (
