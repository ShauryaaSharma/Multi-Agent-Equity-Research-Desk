...
1. **Config:** Add the key to `aerd_conf/settings.py` or `.env`.
2. **Client:** Add a small module under `newswire` or `feed_harvester` (e.g. `finnhub_gateway.py`) that returns normalized items (e.g. `{ "title", "url", "sentiment", "source" }`).
3. **Views:** In `newswire/views.py`, call the new client and merge results with existing news (or switch by `?source=finnhub`).
