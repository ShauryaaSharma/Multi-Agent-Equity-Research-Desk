# Autonomous Equity Research Desk — Complete Project Documentation
...
5. [Backend Modules](#5-backend-modules)
   - 5.1 [newswire](#51-newswire)
   - 5.2 [swarm](#52-swarm)
   - 5.3 [cognition](#53-cognition)
   - 5.4 [alpha\_lab](#54-alpha_lab)
   - 5.5 [jolt\_radar](#55-jolt_radar)
   - 5.6 [scorecard](#56-scorecard)
   - 5.7 [crosslink](#57-crosslink)
   - 5.8 [conduits](#58-conduits)
   - 5.9 [local\_integrations](#59-local_integrations)
   - 5.10 [aerd\_conf](#510-aerd_conf)
...
AERD is a full-stack, production-oriented AI financial intelligence platform. It automates the entire analyst workflow:
...
Manual financial news monitoring is slow, subjective, and unscalable. Traders and analysts miss critical signals buried in thousands of news articles daily. AERD provides:
...
├── api_core/                    # Django application (API + AI + Quant)
│   ├── swarm/                 # Multi-agent orchestration pipeline
│   ├── aerd_conf/                 # Django settings, URLs, ASGI, Celery app
│   ├── crosslink/           # Crypto, commodities, FX, geopolitical analysis
│   ├── scorecard/             # Sentiment accuracy metrics + latency benchmarks
│   ├── newswire/             # News ingestion, FinBERT sentiment, DB models
│   ├── cognition/           # LLM layer (LangChain + Groq/OpenAI)
│   ├── local_integrations/     # Broker integrations (Zerodha Kite)
│   ├── conduits/              # Celery task definitions
│   ├── alpha_lab/                  # Signals, backtesting, strategy engine
│   ├── scripts/                # Management utilities
│   ├── jolt_radar/        # Market shock detection + Telegram alerts
│   ├── manage.py
│   ├── requirements.txt
│   ├── Dockerfile
...
│   ├── hooks/                  # Custom React hooks (useLiveTape, etc.)
...
│   └── package.json
│
├── feed_harvester/              # Standalone Python utility for bulk data collection
│   └── src/                   # Offline NLP model experiments
│
├── notes/                       # Supplemental documentation
│   ├── ui-design-notes.md       # Design system reference
│   ├── wiring.md          # Frontend-backend integration guide
│   ├── data-vendors.md         # API options for Indian/US/global markets
│   ├── jolt-radar.md      # Shock predictor setup guide
│   ├── verdict-engine.md  # Intraday trade decision spec
│   └── qa-checks.md         # End-to-end verification checklist
│
├── .env.example                # Root env template (copy to api_core/.env)
├── SHIPPING.md                   # Vercel + Render deployment guide
├── CONSOLE_SETUP.md           # Frontend-specific setup steps
├── WALKTHROUGH.md         # Technical Q&A reference
├── README.md                   # Project overview + quick start
├── stack.edge.yml     # Local Docker (Postgres for edge mode)
├── stack.prod.yml     # Production Docker Compose
├── render.yaml                 # Render Blueprint configuration
└── requirements.txt            # Root-level requirements (mirrors api_core/)
```
...
│                                                                       │
│  ┌─────────────┐ ┌──────────────┐ ┌──────────────┐ ┌─────────────┐ │
│  │ newswire  │ │   swarm/    │ │ cognition/│ │    alpha_lab/   │ │
│  │ (ingestion, │ │ (12-step     │ │ (LangChain + │ │ (signals,   │ │
│  │  FinBERT,   │ │  orchestr.)  │ │  LLM calls)  │ │  backtest,  │ │
│  │  WebSocket) │ │              │ │              │ │  strategy)  │ │
│  └─────────────┘ └──────────────┘ └──────────────┘ └─────────────┘ │
│  ┌─────────────┐ ┌──────────────┐ ┌──────────────┐ ┌─────────────┐ │
│  │  scorecard │ │ crosslink │ │shock_predict.│ │  conduits  │ │
...
| File | Description |
|------|-------------|
| `views.py` | All primary API views: `newswire`, `analyze_sentiment`, `live_ticker`, `chart_data`, `options_chain`, `scanner`, `trade_decision`, `custom_sentiment` |
| `tone_scoring.py` | Lazy-loads FinBERT model; exposes `analyze_sentiment(text)` returning label + confidence |
| `socket_handlers.py` | `DashboardConsumer` WebSocket handler for `ws/dashboard/` endpoint |
| `models.py` | `NewsArticle` Django model (title, content, published\_at, sentiment) |
| `urls.py` | URL routing for all `newswire` endpoints |
...
|------|------------|------|
| `contracts.py` | `BaseAgent` | Abstract base with `run(ctx)` interface |
| `headline_hunter.py` | `NewsScoutAgent` | Scans sentiment distribution, detects spikes |
| `macro_lens.py` | `MacroContextAgent` | Links headlines to macro themes (rates, CPI, GDP, yields) |
| `chartcraft.py` | `TechnicalAgent` | Moving averages, momentum, volatility signals |
| `tape_reader.py` | `MarketReactionAgent` | Predicts market reaction from news + macro |
| `exposure_unit.py` | `RiskAgent` | Flags concentration, spike, and downside risks |
| `roundtable.py` | `BullResearcherAgent`, `BearResearcherAgent`, `RiskCommitteeAgent`, `DebateFacilitatorAgent` | Structured bull vs. bear debate with risk constraints |
| `verdict_unit.py` | `DecisionAgent` | Synthesizes all agent outputs into BUY/SELL/HOLD |
| `ticker_probe.py` | (standalone) | Single-stock deep analysis with sector peer comparison |
| `conductor.py` | `AgentOrchestrator` | Runs all agents sequentially, returns unified `pipeline` output |
| `payload_shapes.py` | Pydantic schemas | Typed output definitions for agent responses |
...
| File | Description |
|------|-------------|
| `model_gateway.py` | LLM client factory — configures Groq (default) or OpenAI; implements all insight generators: `why_sentiment`, `risk_drivers`, `event_impact`, `event_extraction`, `aspect_based_sentiment` |
| `briefings.py` | Entry point: `get_insights(text, label)` — calls all five insight generators |
...
| File | Description |
|------|-------------|
| `triggers.py` | Three sentiment signals: momentum, MA crossover, mean reversion |
| `replay.py` | Backtrader integration — runs price-only vs sentiment-augmented strategies; computes Sharpe, IC, total return |
| `event_replay.py` | Event-driven backtest (news event → price reaction analysis) |
| `playbook_engine.py` | Modular strategy engine for combining multiple signals |
| `playbook_llm.py` | LLM-generated strategy suggestions based on market context |
| `bench_harness.py` | Research-grade benchmark with transaction costs, slippage, India/global symbols |
| `chart_snapshot.py` | Technical analysis snapshot for a given symbol and timeframe |
| `studies.py` | Technical indicators (RSI, MACD, Bollinger Bands, etc.) |
| `registry.py` | Strategy catalog for the strategy engine |
| `replay_smoke_test.py` | Quick validation script for backtesting pipeline |
...
| `sentinel.py` | `ShockAgent` — computes current shock probability and suggests hedges |
| `text_parse.py` | NLP pipeline for extracting signals from RSS headlines |
| `severity.py` | Rule-based scoring of shock probability (0–100) |
| `wire_reader.py` | RSS feed fetcher (feedparser) for live market news |
| `socket_handlers.py` | WebSocket consumer for `ws/shock/` live score stream |
| `jobs.py` | Celery tasks: `poll_and_score` (every 30s during market hours) + `update_eod_feedback` (15:35 IST) |
| `universe.py` | Nifty/BankNifty/Sensex symbol definitions |
| `tg_notifier.py` | Sends Telegram alerts when shock score ≥ 70 |
...
CELERY_BEAT_SCHEDULE = {
    'shock-poll-and-score': {
        'task': 'jolt_radar.jobs.poll_and_score',
        'schedule': 30.0,  # every 30 seconds during market hours
    },
    'shock-eod-feedback': {
        'task': 'jolt_radar.jobs.update_eod_feedback',
...
| `urls.py` | Root URL router (includes `newswire.urls`) |
| `asgi.py` | ASGI application with Channels URL routing for WebSocket paths |
| `celery.py` | Celery app factory |

**Django Installed Apps:**
`daphne`, `newswire`, `cognition`, `swarm`, `alpha_lab`, `conduits`, `scorecard`, `crosslink`, `jolt_radar`, `django_celery_beat`, `rest_framework`, `corsheaders`, plus all standard Django apps.
...
All pages live under `console/app/`. The dashboard requires authentication (password set via `DASHBOARD_PASSWORD` env var).
...
`console/lib/apiClient.ts` — central HTTP client configured with `NEXT_PUBLIC_API_URL` as base URL.

`console/lib/websocket.ts` — `WebSocketClient` class:
...
Feature flags allow toggling major features at runtime without code changes. They are read from environment variables in `aerd_conf/settings.py`.
...
```bash
git clone <repo-url>
cd Autonomous-Equity-Research-Desk
...
```bash
cp .env.example api_core/.env
# Edit api_core/.env with your API keys
```
...
python manage.py replay_jolts --fast --skip-newsapi --indices nifty
```
...
```bash
docker compose -f stack.edge.yml up -d
```

This starts a local PostgreSQL instance. Set `DATABASE_URL` in `api_core/.env` and `console/.env.local` to the local Postgres connection string.
...
```dockerfile
# From api_core/Dockerfile
# Python 3.11 slim → installs requirements.txt → runs build.sh
# Start: daphne -b 0.0.0.0 -p $PORT aerd_conf.asgi:application
```
...
See [SHIPPING.md](./SHIPPING.md) for the full step-by-step guide.
...
1. Add the API key to `aerd_conf/settings.py` and `.env.example`
2. Create a client module under `newswire/` (e.g. `polygon_client.py`) returning normalized items: `{ "title", "url", "sentiment", "source" }`
3. Merge results in `newswire/views.py` fetch logic
4. Add the key to `data-vendors.md` for reference
...
|-------|----------|--------|-----------|
| **FinBERT OOM on Render free tier** | `newswire/sentiment.py` | Cold start may hit 512 MB RAM limit | Upgrade Render instance; or lazy-load with `torch.no_grad()` and CPU offload |
| **Alpha Vantage 25 req/day limit** | `newswire/views.py` | News feed goes stale after 25 calls | Redis caching is critical; do not call the endpoint in a polling loop |
| **Celery not available on Render free tier** | `conduits/tasks.py` | Background news ingestion and shock scoring disabled | Acceptable for demo; use REST polling fallback |
| **SQLite not suitable for production** | `aerd_conf/settings.py` | SQLite does not support concurrent writes well | Set `DATABASE_URL` to use PostgreSQL on Render |

### Known Technical Issues

| Issue | Details |
|-------|---------|
| **Look-ahead bias in backtests** | If news timestamps are not carefully aligned with price bar timestamps, the backtest may inadvertently use future information. The `bench_harness.py` attempts to handle this but review assumptions carefully for any published results. |
...
| **Shock predictor Sensex data noise** | The yfinance data for `^BSESN` (Sensex) can be noisy or incomplete. The `replay_jolts` command recommends running with `--indices nifty` only. |
...
- `aerd_conf/urls.py` has an explicit `newswire` import at the top and also includes `newswire.urls` — this creates a duplicate route for `/api/fetch-news/` which Django resolves by using the first match.
...
| **Backtesting with real commissions** | Medium | Integrate actual NSE/BSE brokerage fee schedules into `bench_harness.py` |
...
| Item | Effort | Description |
|------|--------|-------------|
| **Switch to PostgreSQL for dev** | Low | Use `stack.edge.yml` by default in local dev; eliminate SQLite dependency |
...
- All contributors must copy `.env.example` → `api_core/.env` and fill in their own keys locally.
- Check `.gitignore` before committing: `api_core/.env`, `console/.env.local`, `*.log`, `db.sqlite3`, `node_modules/`, `.next/` must all be excluded.
...
2. Register the app in `INSTALLED_APPS` in `aerd_conf/settings.py`
3. Add URL patterns in the app's `urls.py` and include in `aerd_conf/urls.py`
...
1. Create a new directory under `console/app/dashboard/`
